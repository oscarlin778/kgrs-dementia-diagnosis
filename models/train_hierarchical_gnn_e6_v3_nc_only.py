...
def run_task_dann(df_task, task_pair, teacher_dict, device, seed,
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df_task, strata_arr)):
        ...
        # Only Domain Classifier for NC
        domain_clf_nc = DomainClassifier().to(device)

        all_params = list(model.parameters()) + list(domain_clf_nc.parameters())
        ...
        for epoch in range(EPOCHS):
            model.train(); domain_clf_nc.train()
...
def main():
    ...
    run_experiment(df_full, device, exp_name="E6_v3_nc_only")
