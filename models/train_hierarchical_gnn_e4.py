...
# ── E4: per-task domain adversarial weight ──────────────────────
# NC_vs_MCI benefits most from domain alignment → strengthen
# MCI_vs_AD needs task-specific features → relax
# NC_vs_AD stays at E3 level
...
def run_task_dann(df_task, task_pair, teacher_dict, device, seed,
    ...
    if ckpt_dir is not None and seed_best_state is not None:
        ...
        ckpt_path = os.path.join(ckpt_dir, f"e4_{safe_name}_seed{seed}.pt")
...
def run_task_multi_seed(df_full, task_pair, device):
    ...
    print(f"\n{'='*60}\n  Task: {task_name}  [E4 DANN method=2, λ_domain={lambda_domain}]\n{'='*60}")
    ...
    ckpt_dir = os.path.join(TEACHER_PROBS_DIR, "e4_checkpoints")
...
# 8. Main
# ===============================================================
def run_experiment(df_full, device, exp_name="E4_task_specific_domain"):
    print(f"\n{'#'*70}")
    print(f"  E4 DANN — Task-Specific λ_domain")
    ...
    fig.suptitle('E4 Task-Specific λ_domain - Confusion Matrices', fontsize=16, fontweight='bold')
...
def main():
    print("E4 DANN — Task-Specific Domain Adversarial Weights")
    ...
    exp_metrics = run_experiment(df_full, device, exp_name="E4_task_specific_domain")

    # Compare E3 method2 vs E4
    import json
    ...
    if os.path.exists(e3_path):
        ...
        print(f"\n{'='*70}")
        print("  E3 Method2 vs E4 Comparison")
        print(f"{'='*70}")
        print(f"{'Task':<16} {'E3 AUC':>8} {'E4 AUC':>8}  {'Δ AUC':>8}  {'Winner':>8}")
        print("-"*60)
        for task in ["NC_vs_AD", "NC_vs_MCI", "MCI_vs_AD"]:
            a3 = e3_metrics.get(task, {}).get('auc', float('nan'))
            a4 = exp_metrics.get(task, {}).get('auc', float('nan'))
            delta = a4 - a3
            winner = "E4" if delta > 0 else ("E3" if delta < 0 else "Tie")
            print(f"  {task:<14} {a3:>7.3f}  {a4:>7.3f}  {delta:>+7.3f}   {winner}")
