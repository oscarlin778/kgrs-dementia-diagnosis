...
# ===============================================================
# 2. E9 Decoupled MTL Model Architecture
# ===============================================================
...
class MultiTaskDataset_E9(Dataset):
...
# 5. Training Loop
# ===============================================================
def run_e9_seed(df_train, teacher_probs_all, device, seed, split_path):
    torch.manual_seed(seed); np.random.seed(seed)
    with open(split_path, 'r') as f: unified_split = json.load(f)
    oof_results = {task: [None]*len(df_train) for task in ['nc_ad', 'nc_mci', 'mci_ad']}
    oof_true = {task: [None]*len(df_train) for task in ['nc_ad', 'nc_mci', 'mci_ad']}
    ...
    for fold in range(5):
        ...
        train_ds = MultiTaskDataset_E9(train_df, teacher_probs_all); val_ds = MultiTaskDataset_E9(val_df, teacher_probs_all)
        ...
        model = FNPGNNv8_E9().to(device); domain_clf = DomainClassifier().to(device)
        ...
        for epoch in range(EPOCHS):
            ...
            for b in train_loader:
                ...
                loss = torch.tensor(0.0, device=device); tasks_list = [('nc_ad', out_nc_ad, ALPHA), ('nc_mci', out_nc_mci, BETA), ('mci_ad', out_mci_ad, GAMMA)]
                ...
                loss_ordinal = torch.tensor(0.0, device=device); is_nc, is_mci, is_ad = (dt == 0), (dt == 1), (dt == 2)
                if is_nc.any() and is_mci.any() and is_ad.any():
                    c_nc, c_mci, c_ad = flat[is_nc].mean(0), flat[is_mci].mean(0), flat[is_ad].mean(0)
                    v_axis = c_ad - c_nc; v_norm_sq = torch.sum(v_axis**2) + 1e-8
                    projection = torch.dot(c_mci - c_nc, v_axis) / v_norm_sq
                    loss_ordinal = F.relu(ORDINAL_MARGIN - projection) + F.relu(projection - (1.0 - ORDINAL_MARGIN))
                
                loss_domain = torch.tensor(0.0, device=device); is_nc_batch = (dt == 0)
                if is_nc_batch.any(): loss_domain = F.binary_cross_entropy_with_logits(domain_clf(flat[is_nc_batch], alpha).squeeze(1), dom_lbl[is_nc_batch])
                loss += LAMBDA_DOMAIN * loss_domain + LAMBDA_ORDINAL * loss_ordinal
        ...
        val_subj_to_idx = {get_subject_id(p): i for i, p in enumerate(df_train['matrix_path'])}
        with torch.no_grad():
            for b_v in val_loader:
...
def main():
    ...
    parser.add_argument('--train-csv', type=str, default='/home/wei-chi/Alzheimers_Project/external_data/scripts/downstream/splits/combined_train.csv')
    parser.add_argument('--test-csv', type=str, default='/home/wei-chi/Alzheimers_Project/external_data/scripts/downstream/splits/combined_test.csv')
    parser.add_argument('--split-path', type=str, default='/home/wei-chi/Alzheimers_Project/external_data/scripts/unified_subject_split_trainonly.json')
    ...
    for task in [('NC','AD'), ('NC','MCI'), ('MCI','AD')]:
        safe = f"{task[0]}_vs_{task[1]}"; p = os.path.join(TEACHER_PROBS_DIR, f"teacher_logits_{safe}.npy")
        if os.path.exists(p): teacher_probs_all[safe.lower()] = np.load(p, allow_pickle=True).item()
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"E9 Clean Redo: ALPHA={ALPHA}, BETA={BETA}, GAMMA={GAMMA}, LAMBDA_ORDINAL={LAMBDA_ORDINAL}")
    
    seed_oof_probs = {t: [] for t in ['nc_ad', 'nc_mci', 'mci_ad']}
    all_seed_models = []

    for seed in SEEDS:
        print(f"\n  [Seed {seed}]")
        res, trues, models = run_e9_seed(df_train, teacher_probs_all, device, seed, args.split_path)
    ...
    for t in seed_oof_probs:
        ...
        auc, acc = roc_auc_score(t_true, t_prob), accuracy_score(t_true, (np.array(t_prob)>0.5).astype(int))
        print(f"E9 Clean OOF Task {t}: ACC={acc:.3f}, AUC={auc:.3f}")
        oof_metrics[t] = {"auc": float(auc), "acc": float(acc)}
    
    # 2. Test Set Evaluation
    print("\n--- Evaluating on Held-out Test Set ---")
    test_ds = MultiTaskDataset_E9(df_test, teacher_probs_all)
    test_loader = DataLoader(test_ds, batch_size=1)
    
    # Ensemble predictions across all seeds and folds
    test_probs_all = {t: [] for t in ['nc_ad', 'nc_mci', 'mci_ad']}
    test_true = {t: [] for t in ['nc_ad', 'nc_mci', 'mci_ad']}
    
    model = FNPGNNv8_E9().to(device)
    for seed_idx, seed_models in enumerate(all_seed_models):
        ...
        for fold_idx, state in enumerate(seed_models):
            ...
            with torch.no_grad():
                for b_t in test_loader:
                    out_nc_ad, out_nc_mci, out_mci_ad, _ = model(b_t['x'].to(device), b_t['adj'].to(device))
                    outs = [out_nc_ad, out_nc_mci, out_mci_ad]
                    for i, t in enumerate(['nc_ad', 'nc_mci', 'mci_ad']):
        ...
        for t in seed_test_probs:
            if seed_test_probs[t]:
                # Reshape to (num_folds, num_test_samples) then mean
                n_samples = len(test_true[t])
                probs = np.array(seed_test_probs[t]).reshape(5, n_samples).mean(axis=0)
                test_probs_all[t].append(probs)

    final_test_metrics = {}
    for t in test_probs_all:
        if test_probs_all[t]:
            avg_test_p = np.mean(test_probs_all[t], axis=0)
            auc_t = roc_auc_score(test_true[t], avg_test_p)
            acc_t = accuracy_score(test_true[t], (avg_test_p > 0.5).astype(int))
            cm = confusion_matrix(test_true[t], (avg_test_p > 0.5).astype(int))
            print(f"E9 Clean Test Task {t}: ACC={acc_t:.3f}, AUC={auc_t:.3f}, N={len(test_true[t])}")
            final_test_metrics[t] = {"auc": float(auc_t), "acc": float(acc_t), "n": len(test_true[t]), "cm": cm.tolist()}

    results = {"oof": oof_metrics, "test": final_test_metrics}
    out_dir = "/home/wei-chi/Alzheimers_Project/external_data/scripts/results/E9_clean"
