...
# ===============================================================
# Settings & Hyperparameters (same as base GNN script)
# ===============================================================
CSV_PATHS = [
...
# ── DANN-specific ──────────────────────────────────────────────
LAMBDA_DOMAIN   = 0.3   # domain adversarial weight
...
class fMRIDataset_KD(Dataset):
    def __init__(self, dataframe, teacher_dict=None):
        ...
        for _, row in dataframe.iterrows():
            ...
            # is_nc_or_ad: True for NC/AD subjects (used in Method 2 masking)
...
def run_task_dann(df_task, task_pair, teacher_dict, device, seed,
                  dann_method=1, lambda_domain=LAMBDA_DOMAIN, ckpt_dir=None):
    """
    dann_method=1: domain loss only for NC_vs_AD task
    dann_method=2: domain loss for all tasks; MCI subjects masked out
    """
    ...
    # Whether domain loss applies for this task
    apply_domain = (dann_method == 1 and task_pair == ('NC', 'AD')) or (dann_method == 2)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df_task, strata_arr)):
        ...
        for epoch in range(EPOCHS):
            ...
            for b in train_loader:
                ...
                if apply_domain:
                    if dann_method == 1:
                        # All subjects in this batch contribute to domain loss
                        domain_logit = domain_clf(flat, alpha)
                        loss_domain = F.binary_cross_entropy_with_logits(
                            domain_logit.squeeze(1), dom_lbl)
                    elif dann_method == 2:
                        # Only NC/AD subjects contribute to domain loss
                        if is_ncad.any():
    ...
    if ckpt_dir is not None and seed_best_state is not None:
        ...
        ckpt_path = os.path.join(ckpt_dir, f"dann_m{dann_method}_{safe_name}_seed{seed}.pt")
...
def run_task_multi_seed(df_full, task_pair, device, dann_method=1, lambda_domain=LAMBDA_DOMAIN):
    class_a, class_b = task_pair
    task_name = f"{class_a} vs {class_b}"
    print(f"\n{'='*60}\n  Task: {task_name}  [DANN method={dann_method}]\n{'='*60}")
    ...
    ckpt_dir = os.path.join(TEACHER_PROBS_DIR, "dann_checkpoints")
    seed_probs, seed_attns, seed_flats = [], [], []
    for seed in SEEDS:
        print(f"\n  [Seed {seed}]")
        probs, all_true, attns, flats = run_task_dann(
            df_task, task_pair, teacher_dict, device, seed,
            dann_method=dann_method, lambda_domain=lambda_domain, ckpt_dir=ckpt_dir)
...
# ===============================================================
# 8. Main — runs Method 1 then Method 2
# ===============================================================
def run_experiment(df_full, device, dann_method, exp_name):
    print(f"\n{'#'*70}")
    print(f"  E3 DANN — Method {dann_method}: {exp_name}")
    print(f"  lambda_domain={LAMBDA_DOMAIN}")
    print(f"{'#'*70}")
    ...
    fig.suptitle(f'E3 DANN Method {dann_method} - Confusion Matrices', fontsize=16, fontweight='bold')

    for idx, task in enumerate(tasks):
        (acc, auc, cm, ens_acc, ens_auc, best_w,
         avg_probs, all_true, matrix_paths, fpr, tpr) = run_task_multi_seed(
            df_full, task, device, dann_method=dann_method, lambda_domain=LAMBDA_DOMAIN)
...
def main():
    print("E3 DANN — Gradient Reversal Domain Adversarial Training")
    print(f"DANN lambda={LAMBDA_DOMAIN}, seeds={SEEDS}")
    ...
    # Method 1: domain adversarial only for NC_vs_AD
    metrics_m1 = run_experiment(
        df_full, device,
        dann_method=1,
        exp_name="E3_domain_adversarial/E3_method1"
    )

    # Method 2: domain adversarial for all tasks, MCI subjects masked
    metrics_m2 = run_experiment(
        df_full, device,
        dann_method=2,
        exp_name="E3_domain_adversarial/E3_method2"
    )

    # Cross-method comparison
    print(f"\n{'='*70}")
    print("  E3 Summary: Method 1 vs Method 2")
    print(f"{'='*70}")
    print(f"{'Task':<16} {'M1 AUC':>8} {'M2 AUC':>8}  {'Winner':>8}")
    print("-"*50)
    for task in ["NC_vs_AD", "NC_vs_MCI", "MCI_vs_AD"]:
        a1 = metrics_m1[task]['auc']
        a2 = metrics_m2[task]['auc']
        winner = "Method1" if a1 >= a2 else "Method2"
        print(f"  {task:<14} {a1:>7.3f}  {a2:>7.3f}   {winner}")
