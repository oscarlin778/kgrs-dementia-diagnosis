...
# ===============================================================
# 2. E9 Decoupled MTL Model Architecture (Same as E8)
# ===============================================================
...
# 5. Training Loop
# ===============================================================
def run_e9_seed(df_full, teacher_probs_all, device, seed, ckpt_dir=None):
    ...
    for fold in range(5):
        ...
        for epoch in range(EPOCHS):
            ...
            for b in train_loader:
                ...
                # E9: Centroid Monotonicity Loss
                loss_ordinal = torch.tensor(0.0, device=device)
                is_nc, is_mci, is_ad = (dt == 0), (dt == 1), (dt == 2)
        ...
        for _, row in df.iterrows():
            ...
            try:
                ...
                if diag in ['NC', 'MCI', 'AD']:
                    src = 'ADNI' if ('adni' in m_path.lower() or 'old_dswau' in m_path.lower()) else 'TPMIC'; valid_data.append({'matrix_path': m_path, 'diagnosis': diag, 'source': src}); seen.add(m_path)
            except: continue
    return valid_data
...
def main():
    ...
    print(f"E9 MTL + Ordinal: ALPHA={ALPHA}, BETA={BETA}, GAMMA={GAMMA}, LAMBDA_ORDINAL={LAMBDA_ORDINAL}")
    seed_probs = {t: [] for t in ['nc_ad', 'nc_mci', 'mci_ad']}
    for seed in SEEDS:
        print(f"\n  [Seed {seed}]")
        res, trues = run_e9_seed(df_full, teacher_probs_all, device, seed)
    ...
    for t in seed_probs:
        ...
        auc, acc = roc_auc_score(t_true, t_prob), accuracy_score(t_true, (np.array(t_prob)>0.5).astype(int)); print(f"E9 Task {t}: ACC={acc:.3f}, AUC={auc:.3f}"); exp_metrics[t] = {"auc": float(auc), "acc": float(acc)}
    ser.save_metrics("E9_MTL_Ordinal", exp_metrics); ser.update_comparison_chart()
