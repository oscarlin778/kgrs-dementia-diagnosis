...
# E10: Boost NC_MCI weight to recover from MTL interference
# E8 was ALPHA=0.5, BETA=1.0, GAMMA=0.5  →  NC_MCI signal too weak vs combined NC_AD+MCI_AD
# E9 ordinal loss hurt NC_MCI (-0.019), so we drop it here
# New ratio: NC_MCI gets 2x weight, NC_AD+MCI_AD each reduced to 0.3
...
# 5. Training Loop
# ===============================================================
def run_e10_seed(df_full, teacher_probs_all, device, seed, ckpt_dir=None):
...
def main():
    ...
    print(f"E10 MTL NC_MCI Boost: ALPHA={ALPHA}, BETA={BETA}, GAMMA={GAMMA}")
    seed_probs = {t: [] for t in ['nc_ad', 'nc_mci', 'mci_ad']}
    for seed in SEEDS:
        print(f"\n  [Seed {seed}]")
        res, trues = run_e10_seed(df_full, teacher_probs_all, device, seed)
        for t in res: seed_probs[t].append(np.array([p if p is not None else [0.5, 0.5] for p in res[t]]))
    exp_metrics = {}
    oof_data_to_save = {}
    print("\n=== Threshold Optimization Results (OOF) ===")
    for t in seed_probs:
        avg_p = np.mean(seed_probs[t], axis=0)
        t_true = np.array([v for v in trues[t] if v is not None])
        t_prob = np.array([avg_p[i, 1] for i, v in enumerate(trues[t]) if v is not None])
    ...
    out_dir = "/home/wei-chi/Alzheimers_Project/external_data/scripts/results/E10_MTL_NCMCIBoost"
    os.makedirs(out_dir, exist_ok=True)
    np.save(os.path.join(out_dir, "oof_predictions.npy"), oof_data_to_save)
    
    ser.save_metrics("E10_MTL_NCMCIBoost", exp_metrics); ser.update_comparison_chart()
