...
class FNPGNNv8_MTL(nn.Module):
    def __init__(self, input_dim=NODE_FEAT_DIM, hidden_dim=HIDDEN_DIM, dropout=DROPOUT):
        ...
        self.projector = nn.Sequential(nn.Linear(head_dim, 128), nn.ELU(), nn.Linear(128, 64))
    ...
    def forward(self, x, adj):
        ...
        return self.head_nc_ad(flat), self.head_nc_mci(flat), self.head_mci_ad(flat), flat

# ===============================================================
# 3. DANN Component (Global or Conditional can be used, E7 starts with Global for simplicity)
# ===============================================================
class DomainClassifier(nn.Module):
    def __init__(self, input_dim=N_NETWORKS * HIDDEN_DIM + HIDDEN_DIM):
        super().__init__()
        self.grl = nn.ModuleList([nn.Identity()]) # Dummy placeholder for GRL logic
        self.net = nn.Sequential(nn.Linear(input_dim, 256), nn.ReLU(), nn.Dropout(0.3), nn.Linear(256, 1))
    def forward(self, x, alpha=1.0):
        # Manual GRL implementation for clarity
        x = x.view_as(x); x = x.detach() + (x - x.detach()) * (-alpha) # GRL logic
        return self.net(x)
...
class MultiTaskDataset(Dataset):
    def __init__(self, dataframe, teacher_probs_all=None):
        ...
        for _, row in dataframe.iterrows():
            ...
            if diag == 'NC':  labels['nc_ad'] = 0; labels['nc_mci'] = 0
            elif diag == 'MCI': labels['nc_mci'] = 1; labels['mci_ad'] = 0
            elif diag == 'AD':  labels['nc_ad'] = 1; labels['mci_ad'] = 1
            
            # Teacher probs (optional matching)
            soft = {'nc_ad': torch.zeros(2), 'nc_mci': torch.zeros(2), 'mci_ad': torch.zeros(2)}
            has_soft = {'nc_ad': False, 'nc_mci': False, 'mci_ad': False}
            for task in labels:
                if labels[task] != -1 and teacher_probs_all and task in teacher_probs_all:
                    if subj_id in teacher_probs_all[task]:
                        soft[task] = torch.FloatTensor(teacher_probs_all[task][subj_id]).flip(0)
                        has_soft[task] = True

            adj_z = np.arctanh(np.clip(adj_raw, -0.999, 0.999))
            x_feat = extract_node_features(adj_z); adj_abs = np.abs(adj_z); np.fill_diagonal(adj_abs, 0); k = int(116 * K_RATIO)
            ...
            self.data_cache.append({
                ...
                'labels': labels, 'soft': soft, 'has_soft': has_soft,
                'subj_id': subj_id, 'domain_label': torch.tensor(domain_label, dtype=torch.float32),
                'diagnosis': diag
            })
...
# 5. Training Loop
# ===============================================================
def run_mtl_seed(df_full, teacher_probs_all, device, seed, ckpt_dir=None):
    ...
    for fold in range(5):
        ...
        train_ds = MultiTaskDataset(train_df, teacher_probs_all)
        val_ds   = MultiTaskDataset(val_df, teacher_probs_all)
        
        # Simple random sampler for MTL; balanced sampling is tricky in MTL but can be done
        train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, drop_last=True)
        val_loader   = DataLoader(val_ds, batch_size=1)
        
        model = FNPGNNv8_MTL().to(device); domain_clf = DomainClassifier().to(device)
        ...
        for epoch in range(EPOCHS):
            ...
            for b in train_loader:
                ...
                tasks = [('nc_ad', out_nc_ad, ALPHA), ('nc_mci', out_nc_mci, BETA), ('mci_ad', out_mci_ad, GAMMA)]
                for t_name, t_out, t_w in tasks:
                    t_lbl = b['labels'][t_name].to(device)
                    mask = (t_lbl != -1)
                    if mask.any():
                        loss_ce = ce_loss_fn(t_out[mask], t_lbl[mask])
                        loss += t_w * loss_ce
                        # Distillation (optional)
                        t_soft = b['soft'][t_name].to(device); t_has_soft = b['has_soft'][t_name].to(device)
                        valid_kd = mask & t_has_soft
                        if valid_kd.any():
                            loss += t_w * LAMBDA_KD * nn.KLDivLoss(reduction="batchmean")(F.log_softmax(t_out[valid_kd], dim=1), t_soft[valid_kd])
                
                # Global DANN
                loss_domain = F.binary_cross_entropy_with_logits(domain_clf(flat, alpha).squeeze(1), dom_lbl)
            ...
            # Validation
            model.eval(); v_probs = {t: [] for t in oof_results}; v_true = {t: [] for t in oof_true}
            with torch.no_grad():
...
def main():
    df_full = pd.DataFrame(analyze_data()) # Helper to load all valid subjects
    teacher_probs_all = {}
    ...
    print(f"MTL Experiment: ALPHA={ALPHA}, BETA={BETA}, GAMMA={GAMMA}")
    
    seed_probs = {t: [] for t in ['nc_ad', 'nc_mci', 'mci_ad']}
    for seed in SEEDS:
        print(f"\n  [Seed {seed}]")
        res, trues = run_mtl_seed(df_full, teacher_probs_all, device, seed)
    ...
    for t in seed_probs:
        ...
        print(f"MTL Task {t}: ACC={acc:.3f}, AUC={auc:.3f}")
        exp_metrics[t] = {"auc": float(auc), "acc": float(acc)}
    ser.save_metrics("E7_MTL", exp_metrics); ser.update_comparison_chart()
