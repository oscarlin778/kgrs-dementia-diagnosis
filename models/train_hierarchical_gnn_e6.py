...
# ── E6: Conditional Domain Alignment ────────────────────────────
# We use separate domain classifiers for NC and AD to align 
# sites within each category independently.
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels):
        ...
        pos_mask = (labels == labels.T).float(); pos_mask.fill_diagonal_(0)
...
# ===============================================================
# 5. Dataset (E6: adds diag_type for Conditional DANN)
# ===============================================================
...
class fMRIDataset_KD(Dataset):
    def __init__(self, dataframe, teacher_dict=None):
        self.data_cache = []; self.has_teacher_count = 0
        for _, row in dataframe.iterrows():
            ...
            if diag == 'NC': diag_type = 0
            elif diag == 'MCI': diag_type = 1
            elif diag == 'AD': diag_type = 2
            else: diag_type = -1
            ...
            for i in range(116):
                top_idx = np.argsort(adj_abs[i])[-k:]; adj_mask[i, top_idx] = adj_z[i, top_idx]
            ...
            self.data_cache.append({
                ...
                'diag_type': torch.tensor(diag_type, dtype=torch.long),
            })
...
def run_task_dann(df_task, task_pair, teacher_dict, device, seed,
    ...
    """E6: Conditional Domain Alignment with dual classifiers."""
    torch.manual_seed(seed); np.random.seed(seed)
    labels_arr = df_task['current_task_label'].values
    strata_arr = (labels_arr.astype(str) + '_' + df_task['source'].fillna('TPMIC').values) if 'source' in df_task.columns else labels_arr
    skf = StratifiedKFold(n_splits=N_FOLDS, shuffle=True, random_state=seed)
    contra_loss_fn = SupConLoss(temperature=CONTRA_TEMP)
    all_val_prob, all_val_true, all_val_attn, all_val_flat = [None]*len(df_task), [None]*len(df_task), [None]*len(df_task), [None]*len(df_task)
    seed_best_acc, seed_best_state = 0.0, None

    for fold, (train_idx, val_idx) in enumerate(skf.split(df_task, strata_arr)):
        print(f"    Fold {fold+1}/{N_FOLDS}", end="  ")
        train_df, val_df = df_task.iloc[train_idx].reset_index(drop=True), df_task.iloc[val_idx].reset_index(drop=True)
        train_ds, val_ds = fMRIDataset_KD(train_df, teacher_dict), fMRIDataset_KD(val_df, teacher_dict)
        ...
        # Dual Domain Classifiers for NC and AD
        domain_clf_nc = DomainClassifier().to(device)
        domain_clf_ad = DomainClassifier().to(device)

        all_params = list(model.parameters()) + list(domain_clf_nc.parameters()) + list(domain_clf_ad.parameters())
        ...
        for epoch in range(EPOCHS):
            model.train(); domain_clf_nc.train(); domain_clf_ad.train()
            alpha = get_dann_alpha(epoch, EPOCHS)
            for b in train_loader:
                x, adj, lbl, dom_lbl = b['x'].to(device), b['adj'].to(device), b['label'].to(device), b['domain_label'].to(device)
                soft_lbl, has_soft, dt = b['soft_label'].to(device), b['has_soft'].to(device), b['diag_type'].to(device)
                logits, flat = model(x, adj); proj = F.normalize(model.projector(flat), dim=-1)
                loss_ce = ce_loss_fn(logits, lbl)
                loss_kd = torch.tensor(0.0, device=device)
                valid_kd = (has_soft == True)
                if valid_kd.any(): loss_kd = distillation_loss(logits[valid_kd], soft_lbl[valid_kd])
                loss_contra = contra_loss_fn(proj, lbl)

                # Conditional Domain Alignment
                loss_domain = torch.tensor(0.0, device=device)
                is_nc = (dt == 0); is_ad = (dt == 2)
                if is_nc.any():
                    d_nc = domain_clf_nc(flat[is_nc], alpha)
                    loss_domain += F.binary_cross_entropy_with_logits(d_nc.squeeze(1), dom_lbl[is_nc])
                if is_ad.any():
                    d_ad = domain_clf_ad(flat[is_ad], alpha)
                    loss_domain += F.binary_cross_entropy_with_logits(d_ad.squeeze(1), dom_lbl[is_ad])

                loss = (LAMBDA_CE * loss_ce + LAMBDA_KD * loss_kd + LAMBDA_CONTRA * loss_contra + lambda_domain * loss_domain)
                optimizer.zero_grad(); loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
                torch.nn.utils.clip_grad_norm_(domain_clf_nc.parameters(), 1.0)
                torch.nn.utils.clip_grad_norm_(domain_clf_ad.parameters(), 1.0)
                optimizer.step()
            ...
            with torch.no_grad():
                for b in val_loader:
                    ...
                    vp.append(out.argmax(dim=1).item()); vt.append(b['label'].item())
        ...
        if best_val_acc > seed_best_acc: seed_best_acc, seed_best_state = best_val_acc, {k: v.cpu().clone() for k, v in model.state_dict().items()}

    if ckpt_dir is not None and seed_best_state is not None:
        safe_name = f"{task_pair[0]}_vs_{task_pair[1]}"
        os.makedirs(ckpt_dir, exist_ok=True)
        torch.save(seed_best_state, os.path.join(ckpt_dir, f"e6_{safe_name}_seed{seed}.pt"))
...
def run_task_multi_seed(df_full, task_pair, device):
    class_a, class_b = task_pair; lambda_domain = LAMBDA_DOMAIN_PER_TASK[task_pair]
    print(f"\n{'='*60}\n  Task: {class_a} vs {class_b}  [E6 Conditional DANN, λ={lambda_domain}]\n{'='*60}")
    ...
    df_task = df_task.reset_index(drop=True); teacher_dict = load_teacher_probs(task_pair)
    ckpt_dir = os.path.join(TEACHER_PROBS_DIR, "e6_checkpoints")
    seed_probs, seed_attns, seed_flats = [], [], []
    for seed in SEEDS:
        print(f"\n  [Seed {seed}]")
        probs, all_true, attns, flats = run_task_dann(df_task, task_pair, teacher_dict, device, seed, lambda_domain=lambda_domain, ckpt_dir=ckpt_dir)
        seed_probs.append(probs); seed_attns.append(attns); seed_flats.append(flats)
        print(f"    Seed {seed} OOF acc: {accuracy_score(all_true, probs.argmax(axis=1))*100:.1f}%")
    avg_probs, avg_attn, avg_flats = np.mean(seed_probs, axis=0), np.mean(seed_attns, axis=0), np.mean(seed_flats, axis=0)
    all_pred = avg_probs.argmax(axis=1).tolist()
    acc, cm = accuracy_score(all_true, all_pred), confusion_matrix(all_true, all_pred)
    ...
    except: _fpr, _tpr = np.array([0., 1.]), np.array([0., 1.])
    ...
    for gnn_w in [0.9, 0.8, 0.7, 0.6, 0.5]:
        ens_p = avg_probs.copy()
        for i, sid in enumerate(subj_ids):
            if sid in teacher_dict: ens_p[i] = gnn_w * avg_probs[i] + (1 - gnn_w) * teacher_dict[sid]
        e_acc = accuracy_score(all_true, ens_p.argmax(axis=1))
        try: e_auc = roc_auc_score(all_true, ens_p[:, 1])
        except: e_auc = float('nan')
        if e_acc > best_ens_acc: best_ens_acc, best_ens_auc, best_w = e_acc, e_auc, gnn_w
    if best_w < 1.0: print(f"  Ensemble: GNN×{best_w:.1f} + Teacher×{1-best_w:.1f} → {best_ens_acc*100:.1f}% / AUC {best_ens_auc:.3f}")
    return acc, auc, cm, best_ens_acc, best_ens_auc, best_w, avg_probs, np.array(all_true), df_task['matrix_path'].tolist(), _fpr, _tpr
...
def run_experiment(df_full, device, exp_name="E6_conditional_dann"):
    print(f"\n{'#'*70}\n  {exp_name}\n{'#'*70}")
    tasks = [('NC', 'AD'), ('NC', 'MCI'), ('MCI', 'AD')]; results, exp_metrics, all_oof = {}, {}, {}
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    for idx, task in enumerate(tasks):
        acc, auc, cm, ens_acc, ens_auc, best_w, avg_probs, all_true, matrix_paths, fpr, tpr = run_task_multi_seed(df_full, task, device)
    ...
    ser.save_metrics(exp_name, exp_metrics); ser.update_comparison_chart()
    plt.tight_layout(); os.makedirs(os.path.join(ser.RESULTS_DIR, exp_name), exist_ok=True)
    plt.savefig(os.path.join(ser.RESULTS_DIR, exp_name, 'confusion_matrices.png'), dpi=300); plt.close()
    print(f"\n{'='*60}\n  Final results — {exp_name}\n{'='*60}")
    print(f"{'Task':<16} {'ACC':>8} {'AUC':>8}   {'Ens ACC':>8} {'Ens AUC':>8}")
    for t_n, (acc, auc, _, e_acc, e_auc, _) in results.items():
        print(f"  {t_n:<14} {acc*100:>7.1f}%  {auc:>7.3f}   {e_acc*100:>7.1f}%  {e_auc:>7.3f}")
    return exp_metrics

def main():
    df_full = load_data(); device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    run_experiment(df_full, device, exp_name="E6_conditional_dann")
