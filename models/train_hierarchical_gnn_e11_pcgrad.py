...
# E11 PCGrad Settings
# We use balanced weights to test the effectiveness of PCGrad surgery
TASKS_W = {'nc_ad': 1.0, 'nc_mci': 1.0, 'mci_ad': 1.0}
SEEDS = [42, 123, 456]

# ===============================================================
# 1. Model Architecture (Same as E10 Decoupled MTL)
# ===============================================================
NETWORK_MAP = {
...
# ===============================================================
# 2. PCGrad Implementation
# ===============================================================
class PCGradOptimizer:
    """Implements Projected Conflicting Gradients surgery."""
    ...
    def pcgrad_backward(self, losses):
        """
        losses: List of individual task losses.
        Calculates gradients for each task, projects them if conflicting, and sums them up.
        """
        task_grads = []
        for loss in losses:
            self.optimizer.zero_grad()
            loss.backward(retain_graph=True)
            # Collect current gradients into a flat vector
            grads = []
            for p in self.params:
                if p.grad is not None: grads.append(p.grad.detach().clone().flatten())
                else: grads.append(torch.zeros_like(p).flatten())
            task_grads.append(torch.cat(grads))

        # PCGrad Surgery
        random.shuffle(task_grads)
        num_tasks = len(task_grads)
        final_grads = copy.deepcopy(task_grads)

        for i in range(num_tasks):
            for j in range(num_tasks):
                if i == j: continue
                dot_product = torch.dot(final_grads[i], task_grads[j])
                if dot_product < 0:
                    # Projection: g_i = g_i - (g_i . g_j / ||g_j||^2) * g_j
                    final_grads[i] -= (dot_product / (torch.norm(task_grads[j])**2 + 1e-8)) * task_grads[j]

        # Sum projected gradients
        merged_grad = torch.stack(final_grads).sum(dim=0)

        # Map merged_grad back to parameter .grad
        self.optimizer.zero_grad()
        offset = 0
        for p in self.params:
            numel = p.numel()
...
# ===============================================================
# 3. Data & Utilities (Reuse from E10)
# ===============================================================
...
class MultiTaskDataset_E11(Dataset):
...
# ===============================================================
# 4. Training Function with PCGrad
# ===============================================================
def run_e11_seed(df_full, teacher_probs_all, device, seed):
    ...
    for fold in range(5):
        ...
        train_ds = MultiTaskDataset_E11(train_df, teacher_probs_all); val_ds = MultiTaskDataset_E11(val_df, teacher_probs_all)
        ...
        model = FNPGNNv8_E11().to(device); domain_clf = DomainClassifier().to(device)
        optimizer = torch.optim.AdamW(list(model.parameters()) + list(domain_clf.parameters()), lr=LR, weight_decay=WEIGHT_DECAY)
        pcgrad = PCGradOptimizer(optimizer, list(model.parameters()) + list(domain_clf.parameters()))
        ...
        for epoch in range(EPOCHS):
            ...
            for b in train_loader:
                ...
                for t_name, t_out in [('nc_ad', out_nc_ad), ('nc_mci', out_nc_mci), ('mci_ad', out_mci_ad)]:
                    ...
                    if mask.any():
                        loss_t = TASKS_W[t_name] * ce_loss_fn(t_out[mask], t_lbl[mask])
                        t_soft, t_has_soft = b['soft'][t_name].to(device), b['has_soft'][t_name].to(device); valid_kd = mask & t_has_soft
                        if valid_kd.any(): loss_t += TASKS_W[t_name] * LAMBDA_KD * nn.KLDivLoss(reduction="batchmean")(F.log_softmax(t_out[valid_kd], dim=1), t_soft[valid_kd])
                        task_losses.append(loss_t)
                is_nc = (dt == 0)
                if is_nc.any():
                    task_losses.append(LAMBDA_DOMAIN * F.binary_cross_entropy_with_logits(domain_clf(flat[is_nc], alpha).squeeze(1), dom_lbl[is_nc]))
                
                # PCGrad Surgery!
                if len(task_losses) > 1: pcgrad.pcgrad_backward(task_losses)
                elif len(task_losses) == 1: 
                    pcgrad.zero_grad(); task_losses[0].backward(); torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
                pcgrad.step()
            # Validation
            if (epoch + 1) % 10 == 0:
...
def main():
    ...
    print(f"E11 PCGrad MTL: Balanced Weights {TASKS_W}"); seed_probs = {t: [] for t in ['nc_ad', 'nc_mci', 'mci_ad']}
    for seed in SEEDS:
        print(f"\n  [Seed {seed}]")
        res, trues = run_e11_seed(df_full, teacher_probs_all, device, seed)
        for t in res: seed_probs[t].append(np.array([p if p is not None else [0.5, 0.5] for p in res[t]]))
    exp_metrics = {}
    for t in seed_probs:
        avg_p = np.mean(seed_probs[t], axis=0); t_true = [v for v in trues[t] if v is not None]; t_prob = [avg_p[i, 1] for i, v in enumerate(trues[t]) if v is not None]
        auc, acc = roc_auc_score(t_true, t_prob), accuracy_score(t_true, (np.array(t_prob)>0.5).astype(int))
        print(f"E11 Task {t}: ACC={acc:.3f}, AUC={auc:.3f}"); exp_metrics[t.upper()] = {"auc": float(auc), "acc": float(acc)}
    ser.save_metrics("E11_MTL_PCGrad", exp_metrics); ser.update_comparison_chart()
