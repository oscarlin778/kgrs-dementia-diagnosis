...
"""
    ...
    if has_fmri and has_smri:
        ...
        modality_instruction = (
            ...
            "    而非補償機制——若需提及此區別，請以白話說明（見術語易讀性規則）。\n"
            ...
            "    4. 認知儲備（選填）：僅當病患教育年限資料充分、且影像特徵確實指向此議題時，\n"
            "       以白話說明大腦代償能力對預測的可能影響；否則省略此項，避免空泛描述。\n"
    ...
    prompt = (
        ...
        f"- ⚠️ 術語易讀性：若報告中必須提及「補償機制」或「認知儲備」，"
        f"需立即以括弧附上白話說明（例如：「補償機制（即大腦在部分區域受損時，"
        f"徵召其他腦區維持功能的代償能力）」）。若當前病患的影像特徵不需要討論此概念，"
        f"請勿主動提及，避免造成混淆。\n"
        f"{modality_instruction}"
        f"- 文獻 grounding（重要）：上方【參考醫學文獻】已針對本病患的腦區/網路與預測診斷檢索篩選，"
        f"並以編號 [1]、[2]… 標示。請在【影像分析洞察】與【臨床診斷建議】中，"
        f"將**主要影像發現的臨床意義連結到這些文獻，並以編號標註**（如「…與 DMN 退化相關 [1]」）。\n"
        f"- ⚠️ 只能引用上方清單內的編號文獻，**嚴禁引用清單以外的論文或憑記憶補充作者/年份**；"
        f"且你陳述的事實必須能在所引編號文獻中找到依據，不要做出文獻未支持的宣稱。\n"
        f"- 僅當某一發現確實無對應檢索文獻時，才簡短註明「該發現暫無直接文獻支持」。\n"
        f"- ⚠️ 一致性原則：引用文獻是為了**佐證**本病患的核心臨床推斷，不要因引用而改變判定，"
        f"也不要在同一段落對同一指標出現前後矛盾的描述。\n"
