...
def main():
    ...
    parser.add_argument("--site_filter", type=str, default="all",
                        choices=["all", "ADNI_new", "TPMIC"],
                        help="Train on single-site only (Task 1-A). Forces --no_combat when not 'all'.")
    parser.add_argument("--k_ratio", type=float, default=None,
                        help="Graph sparsity ratio (default: use module-level K_RATIO=0.20)")
    parser.add_argument("--subsample_ad", type=int, default=None,
                        help="Learning-curve: randomly keep only N AD (positive-class) training subjects "
                             "(deterministic per N). Test set unchanged.")
    ...
    site_tag   = f"_site{args.site_filter}" if args.site_filter != "all" else ""
    # single-site training forces no_combat (can't do 2-site ComBat with 1 site)
    if args.site_filter != "all":
        args.no_combat = True
    dim_tag    = f"_dim{args.fusion_dim}" if args.fusion_dim != 20 else ""
    exp_tag    = ("_v2" if args.split_v2 else "") + mod_tag + combat_tag + fmri_tag + aug_tag + site_tag + dim_tag + seed_tag
    ...
    # 1. Load PCAG alignment (KD no longer used). Site is derived directly from
    #    subject_id — verified identical to the old kd 'source' column (0/204 mismatch).
    import re as _re
    sfx = "_v2" if args.split_v2 else ""
    df_tr = pd.read_csv(BASE_DIR / f"pcag_train_aligned{sfx}.csv")
    df_te = pd.read_csv(BASE_DIR / f"pcag_test_aligned{sfx}.csv")
    ...
    if args.subsample_ad is not None:
        pos_idx = df_tr.index[df_tr['bin_label'] == 1].tolist()
        neg_df = df_tr[df_tr['bin_label'] == 0]
        keep = pd.Series(pos_idx).sample(n=min(args.subsample_ad, len(pos_idx)),
                                         random_state=1000 + args.subsample_ad).tolist()
        df_tr = pd.concat([neg_df, df_tr.loc[keep]]).sort_index().reset_index(drop=True)
        print(f"[subsample_ad] kept {len(keep)} positives + {len(neg_df)} negatives "
              f"= {len(df_tr)} train subjects")

    # Task 1-A: single-site filter — applied AFTER label filtering
    if args.site_filter != "all":
        n_before = len(df_tr)
        df_tr = df_tr[df_tr['source'] == args.site_filter].reset_index(drop=True)
        print(f"[site_filter={args.site_filter}] Train: {n_before} → {len(df_tr)} subjects"
              f" (pos={df_tr['bin_label'].sum()}, neg={(1-df_tr['bin_label']).sum()})")
