...
# --- Constants (Must match eval_finetune_fraction_curve.py) ---
HIDDEN_DIM = 128
DROPOUT    = 0.4
K_RATIO    = 0.20
NETWORK_MAP = {
...
# --- Model Definitions (Must match eval_finetune_fraction_curve.py) ---
...
class TPMICDataset(Dataset):
    def __init__(self, df):
        self.subject_ids = df['subject_id'].tolist()
        self.paths = df['matrix_path'].tolist()
        self.labels = df['label'].tolist()
    def __len__(self): return len(self.paths)
    def __getitem__(self, i):
        mat = np.load(self.paths[i])
        feat = extract_node_features(mat)
        adj  = build_adj(mat)
        return self.subject_ids[i], torch.tensor(feat), torch.tensor(adj), torch.tensor(self.labels[i])
...
def main():
    ...
    # Load all TPMIC subjects
    df_tr = pd.read_csv(SPLITS_DIR / "tpmic_train.csv")
    df_te = pd.read_csv(SPLITS_DIR / "tpmic_test.csv")
    ...
    for task, cfg in tasks.items():
        ...
        ckpt_path = CKPT_BASE / f"{task}_finetune_frac1.00.pt"
        ...
        df_task = df_all[df_all['label'].isin(cfg['classes'])].reset_index(drop=True)
        df_task['bin_label'] = (df_task['label'] == cfg['pos']).astype(int)
        
        ds = TPMICDataset(df_task)
        dl = DataLoader(ds, batch_size=1, shuffle=False, num_workers=4)
        
        sids, embs, labels, bin_labels = [], [], [], []
        
        print(f"  Extracting embeddings for {len(ds)} subjects...")
        with torch.no_grad():
            for sid, feat, adj, label in dl:
                flat = model(feat.to(DEVICE), adj.to(DEVICE))
                sids.append(sid[0])
                embs.append(flat.cpu().numpy()[0])
                labels.append(label.item())
                bin_labels.append((label.item() == cfg['pos']) * 1)
        
        out_path = BASE_DIR / "embeddings" / f"gnn_embeddings_tpmic_{task}.npz"
        ...
        print(f"  [SAVED] {out_path} (Shape: {np.array(embs).shape})")
